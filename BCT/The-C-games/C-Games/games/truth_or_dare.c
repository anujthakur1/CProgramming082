...
void truth_or_dare(char *global_name, char *global_username)
{
    ...
    int truth_count = loadQuestions("./db/truth_or_dare/truths.txt", truth_qns); //returns the total numbers of questions count
    ...
    for (int i = 1; i < numPlayers; i++) // Input player names and initialize scores
    {
        ...
        scanf(" %[^\n]", playerNames[i]); //reads the entire line (stops only at Enter).
