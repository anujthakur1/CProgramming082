...
int rock_paper_scissor(char *global_name, char *global_username)
{
    ...
    print_intro_rock_paper_scissor(global_name); // function call
    ...
    while (true)
    {
        ...
        printf("%s: %s\n", global_name, player_choice == 0 ? "Rock" : (player_choice == 1 ? "Paper" : "Scissors")); // nested ternary operator 
    ...
    else if (players_score < computers_score)
    {
        ...
        sprintf(ch_result, "%d", players_score);
        strcat(ch_result, "(Lost)"); // contatinate score and status
    }
    else
    {
        printf("It's a draw!\n");
    ...
    printf("%s: %d\n", global_name, players_score);
    printf("Computer: %d\n", computers_score);
    register_score("Rock Paper Scissor", global_username, ch_result); //calling function
    printf("Press any key to continue...");
    getch();
    system("cls");
    return 0;
